...
User::User() : id(IDGenerator::getInstance()->getNextID()), name(""), email(""), password(""), address(""), phoneNumber("") {}

User::User(const std::string& name, const std::string& email, const std::string& password)
    : id(IDGenerator::getInstance()->getNextID()), name(name), email(email), password(password) {}

User::~User() {}

void User::updateProfile(const std::string& name, const std::string& email, const std::string& address, const std::string& phoneNumber) {
    this->name = name;
    this->email = email;
...
void User::addToCart(const Product& product, long long quantity) {
